...
class ENodeBServiceHandler:
    ...
    def add(self, identifier: str) -> None:
        if identifier not in self.processes:
            command = ["/srsRAN_4G/build/srsenb/src/srsenb", str(get_file_path(identifier, FileType.CONFIG))]
...
class ENodeBService(BaseService[ENodeB, ENodeBCreate, ENodeBUpdate]):
    ...
    def put(self, o: ENodeBCreate) -> ENodeB:
        ...
        """
        template = "misc/srsran/enb_template.conf"
        ...
        config.set("enb", "mme_addr", o.mme_addr)
        config.set("enb", "gtp_bind_addr", o.gtp_bind_addr)
        config.set("enb", "s1c_bind_addr", o.s1c_bind_addr)
